...
# Sarsa_fe = SarsaFeatureSelection(iterations=50, K=10)
# Sarsa_fe.calc(X_all, y_all, clf="XGB", n_times=10, n_splits=10)
# Sarsa_fe.plot_sarsa_log(["max_sco", "max_reward", "sco_std"])
# print(Sarsa_fe.Sarsa_get_bestInfo(miRNA_ID))
# print(print_clf_feas_met(Sarsa_fe.support_, X_all, y_all, "XGB")[0].T)
import pickle
# with open("cache/Sarsa_13clf_result_v17.pikcle","wb") as f:
#     pickle.dump(rst_dct,f)

# with open("cache/Sarsa_12clf_result_v11.pikcle","rb") as f:
#     rst_dct = pickle.load(f)
    
# lst = []
# infotmp = []
# for k in clfs:
#     lst.append(print_clf_feas_met(rst_dct[k].support_, X_all, y_all, k,n_times=50)[0].T)
...
# print_clf_feas_met(rst_dct[k].support_, X_all, y_all, "XGB",n_times=100)[0].T

# for k in rst_dct:
#     maxit = np.argmax(rst_dct[k].reward_store)
#     print(k, rst_dct[k].reward_store[maxit])
#     print("feas:", miRNA_ID[rst_dct[k].support_] )
#     print(rst_dct[k].info_store[maxit])
...
# from grid_search_clf2 import run_grid_search, clf_param_grids
# for k in ['NB', 'RBF-SVM', 'KNN']:
#     idx = rst_dct['NB'].support_
#     clf_ = clfs[k]
#     param_grid = clf_param_grids[k]
#     tmp_mets, tmp_par = run_grid_search(clf_, param_grid, idx, k, X_all, y_all)
#     print(tmp_mets)
